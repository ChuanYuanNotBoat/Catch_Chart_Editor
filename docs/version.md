...
- 当前开发周期：**Beta v1.11.0（开发中，尚未发布）**
- 最新已发布版本：**Beta v1.10.5（2026-08-05）**
- 当前源码中的正式运行时版本仍保持最新发布版本；进入 v1.11.0 发布阶段时再统一更新 CMake、运行时字符串、安装包和标签。

进行中的 v1.11.0 变更见 [history.md](history.md) 顶部。已经发布的 v1.10.5 历史不会接收 v1.11.0 的新增内容。
...
- 首次运行 v1.11.0 开发版后，建议检查所有浮动面板所在显示器和布局恢复结果。
