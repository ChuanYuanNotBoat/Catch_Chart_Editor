...
- 上一版本：**Beta v1.11.1（2026-09-07）**
- 仓库状态：**Release candidate（最后核对 2026-09-19）**
- Git 标签：待发布

当前工作树在 Beta v1.11.2 发布候选上继续维护；本版本变更见 [history.md](history.md) 顶部的 `Beta v1.11.2` 记录，后续维护内容写入顶部 `Unreleased` 段，已冻结的历史版本不回填后续内容。
...
- 布局：多窗口 ADS 与经典四栏布局分别保存和重置；主编辑区是不可关闭/移动/浮动的稳定区域并保留最低可用尺寸；紧凑工具块停靠时保持自然高度、浮动时恢复自由缩放；损坏或不兼容的 ADS 状态会回退默认布局。
...
- 如面板布局异常，先在异常的模式中执行 `View -> Panels -> Reset Panel Layout`，再重新排列并正常退出；该操作不会清除另一模式的布局。
