...
> 适用版本：Beta v1.11.1
> 仓库维护状态：Unreleased（最后核对 2026-09-11）
...
- `Chart Workspace` 主编辑区没有关闭按钮；即使旧版保存了关闭状态，恢复布局时也会自动重新打开。
- 普通面板关闭后可从顶部 `Panels`、`View -> Panels` 或 `Note` / `BPM` / `Meta` 按钮立即重新打开；`Reopen All Closed Panels` 可一次恢复全部关闭面板。
- 关闭紧凑工具块后，其他工具块会保持自然内容高度，不再自动拉长控件；释放空间由 Note/BPM/Meta 等普通编辑面板吸收。仅含工具块的浮动窗口会在底部保留空白。
...
Grid Settings、范围选择、镜像执行、曲线操作、复制、BPM/Meta 和插件执行等动作按钮使用内容驱动的固定尺寸，不会随侧栏或浮动窗口宽度拉伸；需要显示长内容的输入框和下拉框仍会自适应可用宽度。
