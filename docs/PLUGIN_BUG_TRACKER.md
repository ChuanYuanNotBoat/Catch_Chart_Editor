...
**修复方案**：在 `dispatchPluginCanvasInput` 期间加标志位阻止 overlay 查询定时器重入。

**修改文件**：`src/ui/CustomWidgets/ChartCanvas/ChartCanvas.h`、`ChartCanvas.cpp`、`ChartCanvasInteraction.cpp`
...
- **修复方向**：添加节流（单帧最多一次调度，利用 dirty flag + timer 合并）。

### C4 — `const_cast` + `forceRestartProcess` 悬空引用
...
- **修复方向**：延迟 restart 到调用方返回后，或设置标志让下次调用时重建。
...
### H3 — overlay 查询结果无条件覆盖（与 Bug #2 相关）
...
- **修复方向**：用 `m_overlayQueryInCanvasInput` 标志位防止重入覆盖。
...
| 优先级 | 修复项 | 预期效果 |
|--------|--------|----------|
| **P0** | C4: const_cast+forceRestartProcess 悬空引用 | 防闪退 |
| **P0** | Bug #2 / H3: overlay 定时器重入保护 | 解决 Shift 卡顿闪退 |
| **P1** | C3: mouseMove 事件节流 | 大幅度降低 IPC 阻塞频率 |
| **P1** | H4: 空 overlay 不覆盖缓存 | 防止按键时曲线消失 |
| **P1** | H2: canvasOverlays 传入 context | 修复缩放滚动后 overlay 错位 |
| **P2** | M1/M2: 缓存失效策略完善 | 避免永久缓存过期数据 |
| **P2** | C2: 改为异步 IPC | 架构级改进 |
| **P3** | C1: 去除 mutable QProcess | 架构清理 |
| **P3** | H1/H5/H6/M3-M5/L1-L2: 其余改进 | 健壮性/性能优化 |
...
| 日期 | Bug | 修改文件 | 提交状态 |
|------|-----|---------|---------|
| 2026-06-28 | #1 叠加层闪烁 | `src/plugin/ExternalProcessPlugin.cpp` | 未提交 |