...
| [version.md](version.md) | 开发版本、发布版本、兼容性和升级注意事项 |
...
1. **当前开发版本单独记录。** Beta v1.11.0 的新增、修复和重构只写入 `history.md` 顶部的 v1.11.0 开发段。
2. **发布记录冻结。** 已发布版本（当前最新为 Beta v1.10.5）的历史条目只允许修正事实或错字，不追加后续开发内容。
3. **区分开发版与发布版。** `readme.md` 和 `version.md` 同时标出二者；下载链接始终指向最新已发布版本。
