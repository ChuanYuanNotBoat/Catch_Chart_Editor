...
void PlaybackBenchmarkRunner::finishMeasurement()
{
    ...
    const QString failureText = failures.isEmpty()
                                    ? QStringLiteral("none")
                                    : failures.join(',');
    Logger::info(QStringLiteral("BENCHMARK_END output=%1 passed=%2 fps_tick=%3 fps_canvas=%4 fps_preview=%5 paint_p95_ms=%6 window_update_p95_ms=%7 window_update_max_ms=%8 backing_store_update_count=%9 backing_store_update_p95_ms=%10 backing_store_update_max_ms=%11 display_p95_ms=%12 ui_dispatch_p95_ms=%13 skipped_refresh_pct=%14 scroll_step_change_p95_pct=%15 failures=%16 window_updates_per_frame=%17")
                     .arg(outputPath)
                     ...
                     .arg(summary.value("backing_store_update_request_count").toDouble(), 0, 'f', 0)
                     .arg(summary.value("backing_store_update_request_p95_ms").toDouble(), 0, 'f', 3)
                     .arg(summary.value("backing_store_update_request_max_ms").toDouble(), 0, 'f', 3)
    ...
    QTextStream(stdout) << "BENCHMARK_END output=\"" << outputPath
                        ...
                        << " backing_store_update_count="
                        << QString::number(summary.value("backing_store_update_request_count").toDouble(), 'f', 0)
                        << " backing_store_update_p95_ms="
                        << QString::number(summary.value("backing_store_update_request_p95_ms").toDouble(), 'f', 3)
                        << " backing_store_update_max_ms="
                        << QString::number(summary.value("backing_store_update_request_max_ms").toDouble(), 'f', 3)
